...
def create_example(tokens, edits, tokenizer, detect_vocab, max_tokens_len=1024):
    ...
    assert len(labels) == max_tokens_len
    assert len(detect_ids) == max_tokens_len

    feature = {
        'token_ids': int64_list_feature(token_ids),
        'attention_mask': int64_list_feature(attention_mask),
        'labels': bytes_list_feature(labels),
...
def parse_example(example, max_tokens_len=1024):
    feature_desc = {
        ...
        'attention_mask': FixedLenFeature([max_tokens_len], tf.int64),
        'labels': FixedLenFeature([max_tokens_len], tf.string),
        'detect_ids': FixedLenFeature([max_tokens_len], tf.int64)
    }
    return parse_single_example(example, feature_desc)
...
def bytes_list_feature(value):
    ...
    return Feature(bytes_list=BytesList(value=[x.encode() for x in value]))
